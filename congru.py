...
class CGRU_cell(nn.Module):
    ...
    def __init__(self, input_chans, num_features, filter_size, stride=1, padding = 0):
        ...
        self.ConvGates_input = nn.Conv2d(self.input_chans, 2 * self.num_features, self.filter_size, stride=self.stride,
                                   padding=self.padding)

        self.ConvGates_hidden = nn.Conv2d(self.num_features, 2 * self.num_features, self.filter_size, stride=1,
                              padding=int((filter_size - 1) / 2))

        self.Conv_ct_input = nn.Conv2d(self.input_chans, self.num_features, self.filter_size, stride=self.stride,
                                   padding=self.padding)

        self.Conv_ct_hidden = nn.Conv2d(self.num_features, self.num_features, self.filter_size, stride=1,
                              padding=int((filter_size - 1) / 2))
    ...
    def forward(self, input, hidden_state):
        ...
        c1_input = self.ConvGates_input(input)
        c1_hidden = self.ConvGates_hidden(hidden)
        (rt_input, ut_input) = c1_input.chunk(2, 1)
        (rt_hidden, ut_hidden) = c1_hidden.chunk(2, 1)

        reset_gate = f.sigmoid(rt_input+rt_hidden)
        update_gate = f.sigmoid(ut_input+ut_hidden)
        gated_hidden = torch.mul(reset_gate, hidden)

        # p1 = self.Conv_ct(torch.cat((input, gated_hidden), 1))
        p1_input = self.Conv_ct_input(input)
        p1_hidden = self.Conv_ct_hidden(gated_hidden)

        ct = f.tanh(p1_input+p1_hidden)
...
class CGRU(nn.Module):
    ...
    def __init__(self, input_chans, num_features, filter_size, stride, padding=0, num_layers=1):
        ...
        cell_list = []
        cell_list.append(
            CGRU_cell(self.input_chans, self.num_features, self.filter_size, self.stride, self.padding).cuda())  # the first
        # one has a different number of input channels

        for idcell in range(1, self.num_layers):
            cell_list.append(CGRU_cell(self.num_features, self.num_features, self.filter_size, self.stride, self.padding).cuda())
