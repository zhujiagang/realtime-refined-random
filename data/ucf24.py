...
def make_lists(rootpath, imgtype, split=1, fulltest=False, videos=None, istrain = True, randomsample = 10):
    ...
    for vid_1, videoname in enumerate(videos):
        ...
        if fulltest:
            ...
            lastf = numf
        possible_frame_nums = []
        possible_frame_nums = np.arange(0, lastf, 1)
        if not istrain:
            step = ratios[actidx] * 2.0
            possible_frame_nums = np.arange(0, lastf, step)
        else:
            if randomsample > 0:
                if randomsample > lastf:
                    randomsample = lastf
                indexList = range(lastf)
                import random
                randomIndex = random.sample(indexList, randomsample)
                randomIndex.sort()
                possible_frame_nums = np.array(randomIndex)
            else:
                step = ratios[actidx] * 2.0
                possible_frame_nums = np.arange(0, lastf, step)
